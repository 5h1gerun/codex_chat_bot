...
-   Codex失敗時の詳細ログは `codex_error_logs/` にtxt保存（Discordへはパスのみ返却）
...
※ Discord上で実行ログの保存・表示は行いません（失敗時のみ `codex_error_logs/` に保存）。
